%%
[[Tsurigism]]
[[Mogyr]]
[[Mogyri Clans]]
%%
<div class="wiki-infobox">
    <h4 class="wiki-header">Temple of Yl-Lhatsi</h4>
    <div class="wiki-row">
        <strong>Government</strong>
        <span>Tribal Theocracy</span>
    </div>
    <div class="wiki-row">
        <strong>Head of State</strong>
        <span>Mycomother Lingpa Suolang</span>
    </div>
    <div class="wiki-row">
        <strong>Capital</strong>
        <span>Yl-Lhatsi</span>
    </div>
    <div class="wiki-row">
        <strong>Heraldic Symbol</strong>
        <span>The violet Lingpa Marasmius </span>
    </div>
    <hr class="wiki-hr">
    <div class="wiki-row">
        <strong>Primary Religion</strong>
        <a href="Tsurigism" class="internal-link">Lhatsurgic Tsurigism</a>
    </div>
    <div class="wiki-row">
        <strong>Primary Culture</strong>
        <span><a href="Mogyr" class="internal-link">Mogyr Shroomfolk</a></span>
    </div>
    <div class="wiki-row">
        <strong>Official Language</strong>
        <span>Mogyri</span>
    </div>
    <hr class="wiki-hr">
    <div class="wiki-row">
        <strong>Preceded by</strong>
        
    </div>
    <div class="wiki-row">
        <strong>Formed</strong>
        <span></span>
    </div>
    <img src="https://raw.githubusercontent.com/LoopSimon/Garmeley/refs/heads/main/Media/Lhatsi_flag.png" style="width: 100%; height: 100%; object-fit: cover; display: block; flex-grow: 1;">
</div>

<p>The Temple of Yl-Lhatsi is the largest organized settlement of the Mogyri people of the Drizan Peninsula, sitting at the edge of Lake Tsolha. While nominally claiming authority over all Mogyri of the Region, in effect the Temples control is mostly restricted to its immediate surroundings and many rivalling noble bloodlines only swear allegiance to its authority in name. <br> The current Mycomother, the highest office of the Temple, is Suolang of the Lingpa.</p>