%% 
[[Bihra]]
[[Rayadan]]
[[Japhal]]
%%
<div class="wiki-infobox">
    <h4 class="wiki-header">Omna of Lisava</h4>
    <div class="wiki-row">
        <strong>Government</strong>
        <span>Durenni Omna</span>
    </div>
    <div class="wiki-row">
        <strong>Head of State</strong>
        <span>Lonma Travamelos I</span>
    </div>
    <div class="wiki-row">
        <strong>Capital</strong>
        <span>Prankabur</span>
    </div>
    <div class="wiki-row">
        <strong>Heraldic Symbol</strong>
        <span>Shield of Lisava</span>
    </div>
    <hr class="wiki-hr">
    <div class="wiki-row">
        <strong>Primary Religion</strong>
        <span><a href="Rayadan" class="internal-link">Rayadan</a></span>
    </div>
    <div class="wiki-row">
        <strong>Primary Culture</strong>
        <span>Durenni <a href="Bihra" class="internal-link">Bihra</a></span>
    </div>
    <div class="wiki-row">
        <strong>Official Language</strong>
        <span>Durenni</span>
    </div>
    <hr class="wiki-hr">
    <div class="wiki-row">
        <strong>Preceded by</strong>
        <span><a href="Japhal" class="internal-link">Japhal</a></span>
    </div>
    <div class="wiki-row">
        <strong>Formed</strong>
        <span>5th century PA</span>
    </div>
    <img src="https://raw.githubusercontent.com/LoopSimon/Garmeley/refs/heads/main/Media/Flags/Lisava_Flag.png" style="width: 100%; height: 100%; object-fit: cover; display: block; flex-grow: 1;">
</div>
<p>The Onma of Lisava is the largest nation on the island of Durennis, and the most powerful successor state to the old power of Japhal. Its situated east of the Bay of Zivac, in the green and arable river lands north of the Durenni Peaks. The Onma is a predominantly Bihra realm, and at a tentative armistice with the Xolor of <a href="Pervantes" class="internal-link">Pervantes</a> to its east, following almost a century of War with them and their predecessors, the maritime Empire of <a href="Vanpur" class="internal-link">Vanpur</a>. </p>
<p>Lisava is often seen as the centre for Durenni culture and the religious home of the Rayadan, as was Japhal before it and is allied to the Onma of Zilkand to its west across the Bay. The Capital of Lisava, the great City of Prankabur, is said to hold the most advanced facility for medical learning and the study of the body of Bihra, Sunborn and Human in the entire world.</p>