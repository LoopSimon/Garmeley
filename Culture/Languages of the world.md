
<div class=grid-2x2>
	<div class="grid-item">
	<h3>Vihri Languages</h3>
		<h4><a href="Vihr" class="internal-link">Vihri</a></h4>
		<p>National Language of Vihr and the clerical language of Gethricism</p>
		<h4><a href="Grachy" class="internal-link">Grachi</a></h4>
		<p>National Language of Grachy</p>
	
		<h4><a href="Wermia" class="internal-link">Wermian</a></h4>
		
		Spoken still in various Wermian villages in modern day Vihr
	</div>
	
	<div class="grid-item">
	<h3>Reani</h3>
	
		<p>The language of the Reani tribes that is now mostly spoken in Hainach, Grevenna and eastern Garmeley.</p>
	
	<h4>Lower Urien</h4>
	
		<p>Not just a dialect but now a distinct tongue, the language spoken in Uhl and north of Gelmsburg.</p>
		
	  </div>
	  
	  <div class="grid-item">
		<h3>Urien Languages</h3>
		
		<h4>Old Urien</h4>
			<p>Now mostly extinct, old Urien was the language of the unified Urien tribes in the early centuries PA when Urland existed.
			</p>
			<p>These various dialects are spoken in the Petty Kingdoms.</p>
		<h4>Urien</h4>
			<p>The closest to a “Lingua Franca” on the Continent, Urien is the national language of Garmeley and the various petty Kingdoms, and spoken in almost all major cities of the continent.
			</p>
	</div>
	
	<div class="grid-item">
	<h3>Meadi Languages</h3>
	
		<h4>High Meadi</h4>
		
			<p>The national Language of Meadrath.</p>
		
		<h4>Weadi</h4>
		
			<p>Spoken by the Blijgrater citizens of the Trinwood slopes.</p>
		
		<h4>Old Meadi</h4>
		
			<p>Precursor Language to High Meadi during the times of the Auguri dominion</p>
	</div>
	<div class="grid-item span-col-2">
		<div class ="dual-column-grid">
			<div class="v-center">
			<h3><a href="Elf" class="internal-link">Elvish</a> Languages</h3>
			
				<h4>High Elvish</h4>
					<p>The Current dominant Language in the Elbae Sultanat.</p>
					
				<h4>Under Elvish</h4>
					<p>Spoken by the Deepelves or Drow of the Underlands beneath the surface</p>
			</div>
			
			<div class="v-center">
			<h4>Ancient Elvish</h4>
				<p>One of if not the single oldest language that can be entirely translated and understood to this day, ancient elvish is spoken usually by elven nobility, international speakers are almost always scholars of arcane and divine magic. </p>
			</div>
		</div>
	</div>
	
	<div class="grid-item span-col-2">
		<div class="dual-column-grid">
			<div class="v-center">
			<h3>Smallfolk Languages</h3>
			
			<h4>Old Gnomish Symbolspeak</h4>
			
			<p>A non-verbal Language that is still used within gnomish craftsmanship but almost entirely lost to the world outside of various Ottian Scholars.</p>
			
			<h4>Ottian Gnomish</h4>
			
			<p>Spoken by almost all Gnomes of the World.</p>
			<h4>Halfling Dialects</h4>
			<p>Spoken by the various Halfling Communities mostly found west of Meadrath</p>
			
			</div>
			
			<div class="v-center">
			<h4>Kortopian Dwarvish</h4>
			
			<p>Spoken by the Dwarves of the Kortopi Mountains.</p>
			
			<h4>Trollback Dwarvish</h4>
			
			<p>Spoken by the Dwarves of the Trollback Mountains.</p>
			
			<h4>Mistspire Dwarvish</h4>
			
			<p>Spoken by the Dwarves of the Mistspire Mountains.</p>
			
			<h4>Beheran Dwarvish</h4>
			
			<p>Spoken by the Dwarves of the Beheran Mountains.</p>
			</div>
		</div>
	</div>
	<div class="grid-item span-col-2">
		<h3><a href="Goblinoid" class="internal-link">Goblinoid</a> Languages</h3>
		<h4>Undergob</h4>
		<p>Spoken by the Goblins living in the Goblin Hive Cities of the Underlands.</p>
		<h4>Overgob</h4>
		<p>Spoken by the Goblins living in the Goblin Caravans travelling the Surface world.</p>
		<h4>Hobgob</h4>
		<p>Spoken by the Hobgoblin Clans roaming Surface and Underlands.</p>
	</div>
	
	<div class="grid-item span-col-2">
		<div class="dual-column-grid">
			<div class="v-center">
			<h3>Miscellaneous Human Languages</h3>
			<h4>Arevi</h4>
				<p>Spoken by the tribesmen of the Arevi Yerkir.</p>
			<h4>Cevoli</h4>
				<p>The National Language of Battvia and in the Paper Cities.</p>
			<h4>Kheri</h4>
				<p>The oldest human Language preceding even Rakhi, spoken in Kherova.</p>
			</div>
			<div class="v-center">
			<h4>Bykmål Dialects</h4>
				<p>Spoken by the various peoples of Trykkerog.</p>
			<h4>Ukioll</h4>
				<p>Spoken by the Ukioll peoples in and south of the Eldveil<p>
			<h4>Locni</h4>
				<p>Spoken by the Locni peoples west of the Lowstep Hills <p>
			</div>
		</div>
	</div>
	<div class="grid-item span-col-2">
		<div class="dual-column-grid">
			<div class="v-center">
			<h3>Miscellaneous Inhuman Languages</h3>
			<h4>Kragi</h4>
			Spoken by the Cyclopi of lost Cyraclion.
			<h4>Qahori</h4>
			Spoken by the Orcs of the Qahori Federation.
			<h4>Satyr</h4>
			Spoken by the Satyrs of lost Cyraclion.
			</div>
			<div class="v-center">
			<h4>Nirydian</h4>
			Spoken by the Kobold Khaganates of eastern Nirydia.
			<h4>Tallspeak</h4>
			Spoken by the ogre clans and trolls of the world, related to Giantspeak.
			<h4>Grippli</h4>
			Spoken by the various Grippli tribes of the world
			<h4>Gnollish</h4>
			Spoken by the Gnolls of Behera.
			</div>
		</div>
	</div>
  
	<div class="grid-item span-col-2">
		<div class=dual-column-grid>
			<div class="v-center">
			<h3>Exotic and lost Languages </h3>
			
			<h4>Rakhi</h4>
			
				<p>Language of the lost Empire of Rakh, usually only spoken by scholars.</p>
			
			<h4>Cyracli</h4>
			
				<p>Language of the Torn City, almost no one in the known world speaks this.</p>
			
			<h4>Giantspeak</h4>
			
				<p>Language of the Giants of the North, only spoken by various druids and scholars in far Grachy and beyond.</p>
			
			<h4>Draconic</h4>
			
				<p>While there are no actual reliable records of communication with Dragons, various dragon cults of Trykkerog and Meadland used to commune in this tongue, which is now exclusively spoken by scholars.</p>
			
			<h4>Abyssal</h4>
			
				<p>The “Forbidden Tongue” used to commune with Daemons, Proteans and Fiends of the Chaotic Plane, colloquially referred to as the Abyss.</p>
			
			<h4>Aeor</h4>
			
				<p>Spoken by the mysterious residents and entities of the Stagnant Plane.</p>
			</div>
			<div class="v-center">
			<h4>Celestial</h4>
			
				<p>Spoken by Spirits of the Positive Plane, there are few divine scholars that fluently speak this language but its script is used in various ancient religions.</p>
			
			<h4>Deadspeak</h4>
			
				<p>Spoken by necromancers and invaders from the Negative Plane such as the Kytoni.</p>
			
			<h4>Druidic</h4>
			
				<p>Spoken by the various druidic circles of the world, some say it is impossible for one to learn this language that is not accepted within a circle.</p>
			
			<h4>Sylvan</h4>
			
				<p>The language of the Fey and various other Spirits of the material and spiritual realm.</p>
			
			<h4>Mertongue</h4>
			
				<p>Spoken by the mysterious residents of the Ocean Floors and the Merfolk of the vast Seas, sometimes by Qahori Seafairers.</p>
			
			<h4>Aboleth</h4>
			
				<p>The mind-warping whispers of the unfathomable horrors of the deepest and darkest corners of the world.</p>
			</div>
		</div>
	</div>
</div>